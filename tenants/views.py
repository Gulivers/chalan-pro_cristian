...
def create_tenant_onboarding(request):
    ...
    try:
        ...
        try:
            ...
            # Usar migrate_schemas para aplicar todas las migraciones al nuevo schema
            call_command('migrate_schemas', schema_name=tenant.schema_name, verbosity=1)
            logger.info(f"✓ Migraciones completadas para schema: {tenant.schema_name}")
        except Exception as e:
            logger.error(f"✗ Error al ejecutar migraciones: {str(e)}", exc_info=True)
            # Si las migraciones fallan, intentar limpiar el tenant creado
            try:
                tenant.delete()
            ...
            return Response({
                'success': False,
                'error': f'Error al ejecutar migraciones para el nuevo tenant: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
