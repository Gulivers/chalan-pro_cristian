      ...
      <div v-if="categoryTotals.length">
        <div class="text-center me-5">
          ...
          <span v-for="(item, index) in categoryTotals" :key="index" class="badge bg-secondary mx-1">
            {{ item.crew__category__name }}: {{ item.total }}
          </span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    updateCalendarPermissions(){
      if (this.hasPermission('appschedule.add_eventdraft')){
    ...
    handleDateClick(info) {
      // console.log('handleDateClick:: ', info.resource.id, info.resource.title)
      // Abre el modal para un nuevo evento
      ...
      }, true);  // Abre en modo edición directamente para nuevos eventos
    },
